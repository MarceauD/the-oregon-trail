    ...
    filteredThreads.forEach((thread) => {
        ...
        <div class="card-actions-bar">
            <div class="button-group">
                ...
                ` : ''}
                <button class="card-button" onclick="exportSingleItem('thread', '${thread.id}')" title="Copier les détails" style="display: ${isReadOnly ? 'none' : 'flex'}">
...
window.togglePlotIdea = async function (id) {
    ...
    if (idea) {
        idea.done = !idea.done;
        await savePartialData('character.plotNotes', gameState.character.plotNotes);
        renderPlotIdeas();
    }
};
...
window.updatePlotIdeaText = function (id, text) {
    ...
    if (idea) {
        ...
        window.plotNotesSaveTimeout = setTimeout(() => savePartialData('character.plotNotes', gameState.character.plotNotes), 1000);
    }
};
